...
f1 = ScalarObjective(name="Maximize Smoothness", evaluator=obj_avg_scenic_beauty)
...
# Pareto Front data
evolver = NSGAIII(problem=prob, n_iterations=100,
                  n_gen_per_iter=100,
                  population_size=100)
...
objective1_pareto = solutions[:, 0]
objective2_pareto = solutions[:, 1]

# Extract objectives for plotting
objective1_non_pareto = y.objectives[:, 0]
objective2_non_pareto = y.objectives[:, 1]
...
plt.xlabel('Distance')
plt.ylabel('Average scenic beauty')
plt.title('Pareto Front and average scenic beauty')
plt.legend()
plt.grid(True)
plt.show()
